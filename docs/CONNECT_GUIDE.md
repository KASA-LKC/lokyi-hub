...
> 無論自動定手動，dashboard 都唔會「自動感應」到新嘢，你要主動更新一次。下面係每次教授加新 assignment / 改堂 / 新郵件時要做嘅動作。

## 情況一：自動連接已經批到（PART A / B 成功）
- **Canvas**：返 dashboard「🎨 Canvas 連接」→ 撳 **「🔄 立即同步」** 一次，全拉最新課程 + 作業。零複製，最慳力。
- **Outlook**：返「📧 Outlook 連接」→ 撳 **「📅 同步行事曆」**（拉最新 4 個月日程）+ **「✉️ 同步郵件資訊」**（掃最新 80 封）。
- ✅ 呢個情況下你完全唔使 copy 任何嘢，撳掣就得。

## 情況二：仲係手動（學校未批，用 PART C）
- **Canvas**：返 Canvas → 入科 → Assignments（或 To Do）→ **重新全選 copy 成個 list** → 返 dashboard「🎨 Canvas 連接」→「📥 手動匯入」→ 貼上 → 撳 **「🔍 解析 Canvas 內容」**。
  - ⭐ 唔使記住邊啲係新嘅 —— 成個 list 再貼一次就得，系統會**自動去重**，舊嘅唔會變兩倍。
- **Outlook 行事曆**：重新 export 個 `.ics`（Outlook 網頁版發佈 ICS link / Apple Calendar 輸出）→ 返「📧 Outlook 連接」→「📥 手動匯入」→ 重新揀檔 → 撳 **「📥 匯入行事曆 (.ics)」**（自動去重）。
- **Outlook 郵件**：將**新收到嘅重要 email** copy 落大格 → 撳 **「🔍 解析郵件內容」**（按主旨去重，舊嘅唔會重複入）。

## 實用建議
- **頻率**：建議每週一次（例如星期日），或者收到教授通知「有 new assignment / 改堂」就即 update。
- **最慳力做法**：優先搞掂 PART A / B 自動連接（send 咗申請信就等回覆），批到之後全部變「撳一掣」。
- **去重放心**：手動匯入重貼幾次都冇副作用，唔會重複 count，所以可以「懶人式」成個 list 再貼。
- **斷開 / 清走**：Canvas 撳「🗑 清除 Token」；Outlook 撳「🚪 斷開」；手動匯入各自有「清除」掣。
